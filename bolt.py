...
def main(config):
    ...
    for i in range(10):
        ...
        for j, (train_index, test_index) in enumerate(skf.split(data, y)):
            ...
            if save_models:
                if not os.path.exists(os.path.join('models_ABIDE',config['atlas'],'BolT2')):
                    os.makedirs(os.path.join('models_ABIDE',config['atlas'],'BolT2'),exist_ok=True)
                torch.save(min_val_loss_model_dict, os.path.join('models_ABIDE',config['atlas'],'BolT2','ABIDE_min_val_loss_model_rs' + str(i) + '_fold' + str(j) + '.pth'))
    ...
    for i in range(10):
        ...
        if save_models:
            if not os.path.exists(os.path.join('models_ABIDE',config['atlas'],'BolT2')):
                os.makedirs(os.path.join('models_ABIDE',config['atlas'],'BolT2'),exist_ok=True)
            torch.save(min_val_loss_model_dict, os.path.join('models_ABIDE',config['atlas'],'BolT2','ABIDE_min_val_loss_model_rs' + str(i) + '.pth'))
    ...
    if save_results:            
        ...
        with open(os.path.join('results_ABIDE',config['atlas'],'ABIDE_BolT2_results.pkl'), "wb") as pickle_file:
