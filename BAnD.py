...
def test(encoder_model, test_data_loader, batch_size, loss_func, window_size, device):
    encoder_model.eval()
    soft = nn.Softmax(dim=-1)
    with torch.no_grad():
        zs = []
        ...
        loss = loss_func(soft(z), F.one_hot(y, num_classes=2).float().to(device))
        z = z[:,-1].cpu().numpy()
        y = y.numpy()
...
def main(config):
    ...
    for i in range(1):
        ...
        for j, (train_index, test_index) in enumerate(skf.split(data, y)):
            ...
            with tqdm(total=epochs, desc='(T)') as pbar:
                for epoch in range(1,epochs+1):
                    ...
                    for batch_idx, (batch_data, batch_labels) in enumerate(train_loader):            
                        loss = train(batch_data.to(device), batch_labels.to(device), encoder_model, optimizer, loss_func)
                        total_loss += loss
                        batch_count += 1
                    scheduler.step()
                    ...
                    if epoch % 5 != 0:
                        ...
                        val_loss,val_auc,val_prob,y_val = test(encoder_model,val_loader,batch_size,loss_func,config['max_length'],device)
                        ...
                        test_loss,test_auc,test_prob,y_test = test(encoder_model,test_loader,batch_size,loss_func,config['max_length'],device)
            ...
            if save_models:
                if not os.path.exists(os.path.join('models_ABIDE','BAnD')):
                    os.makedirs(os.path.join('models_ABIDE','BAnD'), exist_ok=True)
                torch.save(min_val_loss_model, os.path.join('models_ABIDE','BAnD','ABIDE_min_val_loss_model_rs' + str(i) + '_fold' + str(j) + '.pth'))
    ...
    for i in range(10):
        ...
        with tqdm(total=epochs, desc='(T)') as pbar:
            for epoch in range(1,epochs+1):
                ...
                if epoch % 5 != 0:
                    ...
                    if val_loss < min_val_loss:
                        min_val_loss_model = copy.deepcopy(encoder_model.state_dict())
                    test_loss,test_auc,test_prob,y_ext_test = test(encoder_model,test_loader,batch_size,loss_func,config['max_length'],device)
                    test_losses.append(test_loss)
                    test_aucs.append(test_auc)
                    test_probs.append(test_prob)
                    y_tests.append(y_test)
        ...
        if save_models:
            if not os.path.exists(os.path.join('models_ABIDE','BAnD')):
                os.makedirs(os.path.join('models_ABIDE','BAnD'), exist_ok=True)
            torch.save(min_val_loss_model, os.path.join('models_ABIDE','BAnD','ABIDE_min_val_loss_model_rs' + str(i) + '.pth'))
    ...
    if save_results:            
        if not os.path.exists(os.path.join('results_ABIDE')):
            os.makedirs(os.path.join('results_ABIDE'),exist_ok=True)
        
        with open(os.path.join('results_ABIDE','ABIDE_BAnD_results.pkl'), "wb") as pickle_file:
            pickle.dump(results, pickle_file)
    return results

if __name__ == '__main__':
    config = {}
    config['path_data'] = r'/mnt/harddrive/ABIDEI_BAnD'
