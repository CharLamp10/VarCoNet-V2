...
import numpy as np
import torch
from pretext_code.pre_training import train_new
from pretext_code.model import Pretext
from torch.utils.data import DataLoader
from utils import UCGLDataset, UCGLDataset2, BatchSampler, LoadAllUCGL
import os
import pickle
...
def main(config):
    ...
    for key in data_list:
        dat = data_list[key][:137,:]
        data.append((dat - np.mean(dat, axis=0, keepdims=True))/np.std(dat, axis=0, keepdims=True))
    ...
    for i in range(10):
        ...
        for j, (train_index, test_index) in enumerate(skf.split(data, y)):
            ...
            train_dataset = UCGLDataset(train_data, y_train) 
            batch_sampler = BatchSampler(names_train, batch_size=config['batch_size'],shuffle=config['shuffle'])
            train_loader = DataLoader(train_dataset, batch_sampler=batch_sampler)
            train_dataset2 = UCGLDataset2(train_data, y_train)
            train_loader2 = DataLoader(train_dataset2, batch_size=config['batch_size'])
            val_dataset = UCGLDataset2(val_data, y_val)
            val_loader = DataLoader(val_dataset, batch_size=config['batch_size'])
            test_dataset = UCGLDataset2(test_data, y_test)
            test_loader = DataLoader(test_dataset, batch_size=config['batch_size'])  
            names_train_all.append(names_train)
            names_val_all.append(names_val)
            names_test_all.append(names_test)
            
            data_alla1, data_alla2 = LoadAllUCGL(train_data)
            Model = Pretext(config['dim'], config['pred_dim'], data_alla1,data_alla2)
            A = Model.first_extractor.A
            config['A'] = A
            ...
            if config['save_models']:
                if not os.path.exists(os.path.join(config['path_save'],'models_ABIDEI',config['atlas'],'UCGL')):
                    os.makedirs(os.path.join(config['path_save'],'models_ABIDEI',config['atlas'],'UCGL'),exist_ok=True)
                torch.save(min_val_loss_model, os.path.join(config['path_save'],'models_ABIDEI',config['atlas'],'UCGL','min_val_loss_model_rs' + str(i) + '_fold' + str(j) + '.pth'))
                torch.save(A, os.path.join(config['path_save'],'models_ABIDEI',config['atlas'],'UCGL','A_rs' + str(i) + '_fold' + str(j) + '.pt'))
    ...
    for i in range(10):
        ...
        val_dataset = UCGLDataset2(val_data, y_val)
        val_loader = DataLoader(val_dataset, batch_size=config['batch_size'])
        ext_test_dataset = UCGLDataset2(ext_test_data, y_ext_test)
        ...
        if config['save_models']:
            if not os.path.exists(os.path.join(config['path_save'],'models_ABIDEI',config['atlas'],'UCGL')):
                os.makedirs(os.path.join(config['path_save'],'models_ABIDEI',config['atlas'],'UCGL'),exist_ok=True)
            torch.save(min_val_loss_model, os.path.join(config['path_save'],'models_ABIDEI',config['atlas'],'UCGL','min_val_loss_model_rs' + str(i) + '.pth'))
            torch.save(A, os.path.join(config['path_save'],'models_ABIDEI',config['atlas'],'UCGL','A_rs' + str(i) + '.pt'))
    ...
    if config['save_results']:          
        ...
        with open(os.path.join(config['path_save'],'results_ABIDEI',config['atlas'],'ABIDEI_UCGL_results2.pkl'), 'wb') as f:
...
if __name__ == '__main__':   
    ...
    parser = argparse.ArgumentParser(description='Run UCGL on ABIDE I for ASD classification')
    ...
    parser.add_argument('--pred_dim', type=int, default=32,
