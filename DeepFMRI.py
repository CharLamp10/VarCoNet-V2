...
def main(config):
    ...
    for key in data_list:
        data.append((data_list[key][:120,:] - np.mean(data_list[key][:120,:],axis=0,keepdims=True))/np.std(data_list[key][:120,:],axis=0,keepdims=True))
    ...
    for i in range(10):
        ...
        for j, (train_index, test_index) in enumerate(skf.split(data, y)):
            ...
            encoder_model = SeqenceModel(roi_num).to(device)
            ...
            if save_models:
                if not os.path.exists(os.path.join('models_ABIDE',config['atlas'],'DeepFMRI')):
                    os.makedirs(os.path.join('models_ABIDE',config['atlas'],'DeepFMRI'),exist_ok=True)
                torch.save(min_val_loss_model, os.path.join('models_ABIDE',config['atlas'],'DeepFMRI','ABIDE_min_val_loss_model_rs' + str(i) + '_fold' + str(j) + '.pth'))
    ...
    for i in range(10):
        ...
        if save_models:
            if not os.path.exists(os.path.join('models_ABIDE',config['atlas'],'DeepFMRI')):
                os.makedirs(os.path.join('models_ABIDE',config['atlas'],'DeepFMRI'), exist_ok=True)
            torch.save(min_val_loss_model, os.path.join('models_ABIDE',config['atlas'],'DeepFMRI','ABIDE_min_val_loss_model_rs' + str(i) + '.pth'))
    ...
    if save_results:            
        ...
        with open(os.path.join('results_ABIDE',config['atlas'],'ABIDE_DeepFMRI_results.pkl'), "wb") as pickle_file:
