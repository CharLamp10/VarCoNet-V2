...
def main(config):
    ...
    for i in range(10):
        ...
        for j, (train_index, test_index) in enumerate(skf.split(data, y)):
            ...
            if config['save_models']:
                if not os.path.exists(os.path.join(config['path_save'],'models_ABIDE',config['atlas'],'VarCoNetV2')):
                    os.makedirs(os.path.join(config['path_save'],'models_ABIDE',config['atlas'],'VarCoNetV2'),exist_ok=True)
                torch.save(min_val_loss_model, os.path.join(config['path_save'],'models_ABIDE',config['atlas'],'VarCoNetV2','ABIDE_min_val_loss_model_rs' + str(i) + '_fold' + str(j) + '.pth'))
                torch.save(min_val_loss_classifier, os.path.join(config['path_save'],'models_ABIDE',config['atlas'],'VarCoNetV2','ABIDE_min_val_loss_classifier_rs' + str(i) + '_fold' + str(j) + '.pth'))
    ...
    for i in range(10):
        ...
        if config['save_models']:
            if not os.path.exists(os.path.join(config['path_save'],'models_ABIDE',config['atlas'],'VarCoNetV2')):
                os.makedirs(os.path.join(config['path_save'],'models_ABIDE',config['atlas'],'VarCoNetV2'),exist_ok=True)
            torch.save(min_val_loss_model, os.path.join(config['path_save'],'models_ABIDE',config['atlas'],'VarCoNetV2','ABIDE_min_val_loss_model_rs' + str(i) + '.pth'))
            torch.save(min_val_loss_classifier, os.path.join(config['path_save'],'models_ABIDE',config['atlas'],'VarCoNetV2','ABIDE_min_val_loss_classifier_rs' + str(i) + '.pth'))
    ...
    if config['save_results']:          
        if not os.path.exists(os.path.join(config['path_save'],'results_ABIDE',config['atlas'])):
            os.makedirs(os.path.join(config['path_save'],'results_ABIDE',config['atlas']),exist_ok=True)
        with open(os.path.join(config['path_save'],'results_ABIDE',config['atlas'],'ABIDE_VarCoNetV2_results.pkl'), 'wb') as f:
...
if __name__ == '__main__':   
    ...
    parser.add_argument('--path_data', type=str, default='/home/student1/Desktop/Charalampos_Lamprou/SSL_FC_matrix_GNN_data/ABIDE/fmriprep',
