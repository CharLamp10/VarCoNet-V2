...
class Transformer(nn.Module):
    def __init__(self, d_model, n_layers, n_heads, dim_feedforward, max_len, add_rois):
        super(Transformer, self).__init__()
        self.d_model = d_model
        encoder_layer = nn.TransformerEncoderLayer(d_model+add_rois, n_heads, dim_feedforward, batch_first = True)
        ...
        new_len = (max_len - 8) // 4 + 1 #add_rois
        self.cls_token = torch.nn.Parameter(torch.randn(1, new_len, add_rois)) #add_rois
    ...
    def forward(self, x, x_mask=None):   
        ...
        x_mask = (x[:, :, 0] == 0).bool()    
        x = self.pos_enc(x)
        cls_token = self.cls_token.expand(x.shape[0], -1, -1) #add_rois
        x = torch.cat([x,cls_token],dim=-1) #add_rois
        x[x_mask,:] = 0
...
class SeqenceModel(nn.Module):
    def __init__(self, model_config, roi_num):
        ...
        self.extract = Transformer(
                ...
                add_rois=model_config['add_rois'])
        roi_num_new = roi_num + model_config['add_rois']
        self.linear = nn.Sequential(nn.Linear(int(roi_num_new*(roi_num_new-1)/2), 2),
                                    nn.Softmax(dim=-1))
...
def main(config):
    ...
    for i in range(10):
        ...
        for j, (train_index, test_index) in enumerate(skf.split(data, y)):
            ...
            if save_models:
                if not os.path.exists(os.path.join('models_ABIDE',config['atlas'],'SL')):
                    os.makedirs(os.path.join('models_ABIDE',config['atlas'],'SL'), exist_ok=True)
                torch.save(min_val_loss_model, os.path.join('models_ABIDE',config['atlas'],'SL','ABIDE_min_val_loss_model_rs' + str(i) + '_fold' + str(j) + '.pth'))
    ...
    for i in range(10):
        ...
        if save_models:
            if not os.path.exists(os.path.join('models_ABIDE',config['atlas'],'SL')):
                os.makedirs(os.path.join('models_ABIDE',config['atlas'],'SL'), exist_ok=True)
            torch.save(min_val_loss_model, os.path.join('models_ABIDE',config['atlas'],'SL','ABIDE_min_val_loss_model_rs' + str(i) + '.pth'))
    ...
    if save_results:            
        ...
        with open(os.path.join('results_ABIDE',config['atlas'],'ABIDE_SL_results.pkl'), "wb") as pickle_file:
...
if __name__ == '__main__':
    ...
    with open('best_params_VarCoNet_v2_add_rois_' + config['atlas'] + '.pkl','rb') as f:
    ...
    config['model_config']['add_rois'] = best_params['add_rois']   
